document.addEventListener("DOMContentLoaded", () => {
    ...
    const favButtons = document.querySelectorAll(".luxury-fav-btn, .favorite-btn");
    favButtons.forEach(btn => {
        ...
        if (favorites.some(item => item.model === carId && item.brand === carBrand)) {
        ...
        btn.addEventListener("click", () => {
            ...
            if (btn.classList.contains("active")) {
                ...
                favorites = favorites.filter(item => !(item.model === carId && item.brand === carBrand));
            }
            saveFavorites();
        });
    });
