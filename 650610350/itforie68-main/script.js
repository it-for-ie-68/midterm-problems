class HighlightSearch {
    ...
    // Add searchable data to existing elements
    addDataAttributes() {
        const brandData = {
            'ferrari': {
                category: 'hypercar',
                keywords: 'italian red prancing horse supercar exotic racing f1 formula one',
                models: 'sf90 spider laferri'
            },
            'lamborghini': {
                category: 'hypercar', 
                keywords: 'italian bull green yellow supercar exotic racing aventador huracan',
    ...
    highlightMatches(row, query) {
        ...
        if (!row.hasAttribute('data-original-saved')) {
            ...
            links.forEach((link, index) => {
                ...
                row.setAttribute(`data-original-text-${index}`, link.textContent.trim());
            });
    ...
    updateSearchStats(totalMatches, matchedRows) {
        ...
        if (totalMatches > 0) {
            statsDiv.innerHTML = `
                🎯 Found <strong>${matchedRows}</strong> matching car${matchedRows > 1 ? 's' : ''} 
                with <strong>${totalMatches}</strong> match${totalMatches > 1 ? 'es' : ''}
            `;
...
class Slideshow {
    ...
    showSlide(index) {
        ...
        this.currentSlide = (index + this.slideImages.length) % this.slideImages.length;
